...
class PubMedPostgreSQLDatabaseController:
    '''
    classdocs
    '''
    ...
    def run_sql_query(self, sql_query_in, sql_params_in = None):
        ...
        return run_sql_query_local(sql_query_in, local_sql_params_in=sql_params_in)

class PubMedPostgreSQLDatabaseSearch(PubMedPostgreSQLDatabaseController):
                ...
                try:
                    self.run_sql_query("""
                                                    CREATE TABLE tbl_CTSA_Grant_Searches (
                                                        CTSA_Grant_Search_ID SERIAL UNSIGNED NOT NULL,
                                                        CTSA_Grant_Number_ID INT,
...
class PubMedPostgreSQLDatabaseManager(PubMedPostgreSQLDatabaseController):
    ...
    database_table_parts = [
                            {
                             ...
                             'tableColumns' : """
                                                 ...
                                                 PubMed_XML TEXT,
                                                 PubMed_XML_Date TIMESTAMPTZ"""},
                            {
                             'tableName' : 'tbl_Investigator_Publications',
                             'tableColumns' : """
                                                 Investigator_Publications_ID SERIAL PRIMARY KEY,
                                                 Investigator_ID INT,
                                                 Publication_ID INT"""},
                            {
                                                 ...
                                                 CTSA_Grant_Number_ID INT"""},
                            {
                             ...
                             'tableColumns' : """
                                                 Publication_Grants_Cited_ID SERIAL NOT NULL PRIMARY KEY,
                                                 Publication_ID INT,
                                                 Grant_ID INT"""},
                            {
                             'tableName' : 'tbl_Coauthors',
                             'tableColumns' : """
                                                 Coauthor_ID SERIAL NOT NULL PRIMARY KEY,
                                                 LastName VARCHAR(64),
    ...
    def delete_publications_tables(self):
        ...
        for database_table_item in self.database_table_parts:
            ...
            try:
                ...
                print('Drop table skipped: ' + database_table_item['tableName'])
    ...
    def create_publications_tables(self):
        ...
        for database_table_item in self.database_table_parts:
            ...
            try:
                print("CREATE TABLE " + database_table_item['tableName'] + "(" + database_table_item['tableColumns'] + ");")
                ...
                print('Create table failed: ' + database_table_item['tableName'])
    ...
    def lookup_publication_id_by_pubmed_id(self, pubmed_id_in):
        ...
        try:
            self.run_sql_query(
                "SELECT Publication_ID FROM tbl_PubMed_Publications WHERE PubMed_ID = %s;",
                (str(query_pubmed_id),)
            )
    ...
    def get_coauthor_id(self, last_name_in, first_name_in, initials_in, affiliation_in):
        ...
        if last_name_in:
            sql_query += " UPPER(Trim(%s)) = UPPER(Trim(LastName)) "
        ...
        if first_name_in:
            sql_query += " AND UPPER(Trim(%s)) = UPPER(Trim(FirstName)) "
            db_parameters.append(
                first_name_in
            )
        else:
        ...
        if initials_in:
            sql_query += " AND UPPER(Trim(%s)) = UPPER(Trim(Initials)) "
            db_parameters.append(
                initials_in
            )
        else:
        ...
        if affiliation_in:
            sql_query += " AND UPPER(Trim(%s)) = UPPER(Trim(Affiliation)) "
            db_parameters.append(
                affiliation_in
            )
        else:
        ...
        if not return_coauthor_id:
            ...
            self.run_sql_query("INSERT INTO tbl_Coauthors (" + sql_query_fields + ") VALUES (" + sql_query_values + ") RETURNING Coauthor_ID;", db_parameters)
            self.database_connection.commit()
    ...
    def get_mesh_descriptor_id (self, mesh_descriptor_in):
        ...
        sql_query = """
                       ...
                       WHERE UPPER(Trim(Mesh_Descriptor)) = UPPER(Trim(%s))
                       ;"""
        ...
        if mesh_descriptor_id == None:
            ...
            self.run_sql_query("INSERT INTO tbl_Mesh_Descriptors (Mesh_Descriptor) VALUES (%s) RETURNING Mesh_Descriptor_ID;", (mesh_descriptor_in,))
            self.database_connection.commit()
    ...
    def get_mesh_qualifier_id (self, mesh_qualifier_in):
        ...
        sql_query = """
                       ...
                       WHERE UPPER(Trim(Mesh_Qualifier)) = UPPER(Trim(%s))
                       ;"""
        ...
        if mesh_qualifier_id == None:
            ...
            self.run_sql_query("INSERT INTO tbl_Mesh_Qualifiers (Mesh_Qualifier) VALUES (%s) RETURNING Mesh_Qualifier_ID;", (mesh_qualifier_in,))
            self.database_connection.commit()
    ...
    def get_grant_id(
        ...
        if return_grant_id == None:
            ...
            self.run_sql_query("INSERT INTO tbl_Grants (" + sql_query_fields + ") VALUES (" + sql_query_values + ") RETURNING Grant_ID;", db_parameters)
            self.database_connection.commit()
